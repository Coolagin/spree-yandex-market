...
module Export
  class YandexMarketExporter
        ...
        xml.yml_catalog(:date => Time.now.to_s(:ym)) {
          ...
          xml.shop { # описание магазина
                     ...
                     xml.offers { # список товаров
                                  ...
                                  products = Spree::Product.where(:export_to_yandex_market => true).group("#{Spree::Product.table_name}.id")
    ...
    def offer(xml,product)
      opt = { :type => 'vendor.model', :id => product.id, :available => (product.master.count_on_hand > 0) }
      xml.offer(opt) {
